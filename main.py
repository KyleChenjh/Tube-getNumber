...
H1 = None
# 重定义数码管位段列表
out1 = [H1, G1, F1, E1, D1, C1, B1, A1]
out2 = [H2, G2, F2, E2, D2, C2, B2, A2]
out3 = [H3, G3, F3, E3, D3, C3, B3, A3]
...
def get_value():
    ...
    try:
        ...
        for i in range(1, 8):
            if out2[i].value() == 0:  # 检测电平为高时，说明数码管是熄灭的状态,因此只需检测是否为0即可
                out2_value |= 1
            out2_value <<= 1
        out2_value >>= 1
        ...
        for i in range(1, 8):
            if out3[i].value() == 0:  # 检测电平为高时，说明数码管是熄灭的状态,因此只需检测是否为0即可
                out3_value |= 1
            out3_value <<= 1
        out3_value >>= 1
